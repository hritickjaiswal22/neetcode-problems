...
function rob(root) {
  function find(node, stoleFromParent) {
    if (!node) return 0;
    if (map.get(node)) {
      const obj = map.get(node);

      if (obj[stoleFromParent]) return obj[stoleFromParent];
    }

    let result = 0;

    if (stoleFromParent) {
      result = find(node.left, false) + find(node.right, false);
    } else {
      let temp1 = node.val + find(node.left, true) + find(node.right, true);
      let temp2 =
        find(node.left, stoleFromParent) + find(node.right, stoleFromParent);

      result = Math.max(temp1, temp2);
    }

    const obj = map.get(node) ? map.get(node) : {};
    obj[stoleFromParent] = result;
